      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6 shadow-sm">
          <div className="flex gap-3 mb-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by machine type, capability, or location..." value={query} onChange={e => setQuery(e.target.value)} className="pl-10 h-11 border-gray-200 rounded-lg" data-testid="input-search" />
            </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Select>
              <SelectTrigger className="h-8 w-32 text-sm border-gray-200"><SelectValue placeholder="Price Range" /></SelectTrigger>
              <SelectContent>
            ...
            <Select>
              <SelectTrigger className="h-8 w-28 text-sm border-gray-200"><SelectValue placeholder="Location" /></SelectTrigger>
              <SelectContent>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Select value={sortBy} onValueChange={setSortBy}>
            <SelectTrigger className="h-8 w-40 text-sm border-gray-200 bg-white"><SelectValue /></SelectTrigger>
            <SelectContent>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {m.verified && <span className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-full"><CheckCircle className="w-3 h-3" /> Verified</span>}
                        {m.instantBook && <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full"><Zap className="w-3 h-3" /> Instant Book</span>}
                      </div>
                  ...
                  <div className="mt-2.5">
                    ...
                    <div className="inline-flex flex-wrap gap-1.5 mt-1">
                      {m.availableMachines.map(am => <span key={am} className="text-xs bg-blue-50 text-blue-700 border border-blue-100 px-2 py-0.5 rounded-full">{am}</span>)}
                    </div>
                  </div>
                  <div className="mt-3 flex items-center gap-5 flex-wrap text-xs text-gray-500">
                    ...
                    <div><span className="block text-gray-400">Open Slots</span><span className="font-medium text-gray-700">{m.openSlots} slots</span></div>
                    <div><span className="block text-gray-400">Next Available</span><span className="font-medium text-gray-700">{m.nextAvailable}</span></div>
                    <div><span className="block text-gray-400">Lead Time</span><span className="font-medium text-gray-700">{m.leadTime}</span></div>
                    <div><span className="block text-gray-400">Response Time</span><span className="font-medium text-gray-700">{m.responseTime}</span></div>
                  </div>
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <Link href={`/manufacturer/${m.id}`}>
                      <Button variant="outline" size="sm" className="border-gray-200 text-gray-700 hover:bg-gray-50 font-medium" data-testid={`button-view-profile-${m.id}`}>View Profile</Button>
                    </Link>
