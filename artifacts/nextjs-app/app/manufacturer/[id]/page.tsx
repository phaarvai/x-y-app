...
export default function ManufacturerPage() {
  ...
  const manufacturer = manufacturers.find(m => m.id === parseInt(id || "0"));
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/browse"><Button variant="outline" size="sm" className="border-gray-200 text-gray-700">Back to Search</Button></Link>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {m.verified && <span className="inline-flex items-center gap-1 text-xs font-medium bg-white/20 border border-white/30 text-white px-2 py-0.5 rounded-full"><CheckCircle className="w-3 h-3" /> Verified</span>}
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {m.capabilities.map(cap => (
                  <div key={cap} className="flex items-center gap-2 text-sm text-gray-700 py-2 border-b border-gray-50">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" /> {cap}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              <h2 className="font-bold text-gray-900 text-sm mb-4 flex items-center gap-2"><Award className="w-4 h-4 text-blue-500" /> Certifications</h2>
              <div className="space-y-3">
                {m.certificationDetails.map(cert => (
                  <div key={cert.name} className={`rounded-lg p-3 border ${cert.name.includes("ISO") || cert.name.includes("AS") ? "bg-emerald-50 border-emerald-100" : "bg-gray-50 border-gray-100"}`}>
            ...
            <div className="bg-blue-600 rounded-xl p-6 text-white text-center">
              ...
              <Link href={`/booking/${m.id}/${m.machinery[0]?.id || 1}`}>
                <Button className="w-full bg-white text-blue-700 hover:bg-blue-50 font-semibold">View Availability &amp; Book</Button>
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 bg-white rounded-xl border border-gray-200 p-6 shadow-sm flex items-center justify-between gap-4 flex-wrap">
          ...
          <Link href={`/booking/${m.id}/${m.machinery[0]?.id || 1}`}>
            <Button className="bg-blue-600 hover:bg-blue-700 text-white gap-2 font-semibold px-6">View Availability &amp; Book</Button>
          </Link>
        </div>
      </div>
    </div>
