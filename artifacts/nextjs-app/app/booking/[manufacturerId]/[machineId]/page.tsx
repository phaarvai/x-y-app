...
import { manufacturers, TIME_SLOTS } from "@/lib/manufacturers";
...
export default function BookingPage() {
  ...
  const manufacturer = manufacturers.find(m => m.id === parseInt(manufacturerId || "1"));
  const machine = manufacturer?.machinery.find(m => m.id === parseInt(machineId || "1")) ?? manufacturer?.machinery[0];
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              <div className="flex items-start justify-between mb-1">
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-0.5">{machine.subtitle.split(" • ")[0]}</p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              <h2 className="font-bold text-gray-900 text-base mb-4 flex items-center gap-2"><CalendarDays className="w-5 h-5 text-blue-500" /> Select Your Time Slots</h2>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-5">
                <p className="text-blue-700 text-sm font-medium flex items-center gap-1.5 mb-2"><Info className="w-4 h-4" /> How booking works:</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-medium text-gray-900 text-sm flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-gray-400" />{slot.start} – {slot.end}</p>
